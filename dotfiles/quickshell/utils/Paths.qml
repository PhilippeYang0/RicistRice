...
Singleton {
    ...
    // RicistRice: stands in for the caelestia CLI's `wallpaper` and `scheme`
    // commands (same arguments, same output), using awww + matugen instead.
    // Deployed from dotfiles/ricistrice/bin/theme.sh.
    readonly property string themeCmd: `${Quickshell.env("XDG_CONFIG_HOME") || `${home}/.config`}/ricistrice/bin/theme.sh`
