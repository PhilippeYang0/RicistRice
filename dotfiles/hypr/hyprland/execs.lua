...
-- Dropped from caelestia's list: trash-empty (trash-cli) and the geoclue +
-- gammastep night light, neither of which the base rice uses yet.
hl.on("hyprland.start", function()
    -- Keyring and auth (Cinnamon runs its own polkit agent; Hyprland needs one)
    ...
    -- Wallpaper: awww draws it (the shell's own wallpaper layer is turned off in
    -- ~/.config/caelestia/shell.json); theme.sh restore waits for the daemon,
    -- then puts back the last wallpaper and theme colours
    hl.exec_cmd("awww-daemon")
    hl.exec_cmd(config_home .. "/ricistrice/bin/theme.sh restore")

    -- Start shell (~/.config/quickshell is Quickshell's default config)
    hl.exec_cmd("qs -n -d")
end)
