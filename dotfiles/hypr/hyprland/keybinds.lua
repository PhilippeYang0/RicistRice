...
-- Restore lock (the shell is ~/.config/quickshell, Quickshell's default config,
-- so plain `qs` finds it; -n refuses to start a second copy)
create_bind(vars.kbRestoreLock, function()
    hl.dispatch(hl.dsp.exec_cmd("qs -n -d"))
    hl.dispatch(hl.dsp.global("caelestia:lock"))
end)

-- Kill/restart
create_bind("CTRL + SUPER + SHIFT + R", hl.dsp.exec_cmd("qs kill"), release)
create_bind("CTRL + SUPER + ALT + R", hl.dsp.exec_cmd("qs kill; sleep .1; qs -n -d"), release)
create_bind(vars.kbExit, hl.dsp.exit())

for i = 1, 10 do
    -- Keycodes 10-19 are the number row's physical keys, so these work on
    -- AZERTY too, where the unshifted keys are & é " ' ( - è _ ç à.
    local key = "code:" .. (i + 9)
...
-- Utilities
-- Full screenshot: save to ~/Pictures/Screenshots and copy it (caelestia runs
-- its CLI here; the region/freeze binds below are the shell's own picker)
create_bind(
    vars.kbScreenshot,
    hl.dsp.exec_cmd(
        'mkdir -p ~/Pictures/Screenshots && grim - | tee ~/Pictures/Screenshots/"$(date +%Y%m%d-%H%M%S)".png | wl-copy'
    ),
    locked
)
...
-- Clipboard history (pressing the bind again closes the picker)
create_bind(
    vars.kbClipboard,
...
create_bind(
    vars.kbClipboardDel,
    hl.dsp.exec_cmd("pkill -x rofi || cliphist list | rofi -dmenu -p 'delete from clipboard' | cliphist delete")
)
