...
-- Which apps each special workspace gathers, and how to launch them when they
-- aren't running. caelestia merges overrides from its CLI's cli.json on top of
-- this; there's no caelestia CLI here, so edit this table directly.
local function toggle_config()
    return {
        communication = {
        ...
        sysmon = {
            btop = {
                ...
                command = { "alacritty", "--class", "btop", "--title", "btop", "-e", "btop" },
            },
        },
    }
end
...
local function toggle(special_workspace)
    return function()
        ...
        local apps = toggle_config()[special_workspace]
        if apps then
